...
# Artifact mounts are bind-mounted from the host; create them up front so Docker
# doesn't materialise them as root-owned directories.
mkdir -p admin/test-results admin/playwright-report

# `--no-deps`: rely on `start-dev.sh` having brought up admin/foxnox/traefik.
...
# which on a cold node_modules volume leads to admin crash-looping and Traefik
# returning 404 during the whole poll window.
