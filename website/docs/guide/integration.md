...
|---|---|
| `x-acl-fields` | Filters write rows and projects read rows, history snapshots, and schema fields. Omitted is unrestricted; empty means only `id`. |
| `x-acl-conditions` | Forces search predicates with `AND`, constrains inserts, and checks update/archive/history IDs before access. |
