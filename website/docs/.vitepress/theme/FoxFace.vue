 ...
 * 1280x720 default, and the fox is drawn around a centered square, so a 16:9
...
onMounted(async () => {
  try {
    ...
    const [vertexShader, fragmentShader] = await Promise.all([
      loadShader("fox-face_vert-ready.glsl"),
      loadShader("fox-face_frag-ready.glsl"),
    ]);
    if (disposed) return;
    ...
    // The fox is decoration. A machine without WebGL, or a shader that failed
