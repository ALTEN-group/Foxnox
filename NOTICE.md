...
2. **Stops and removes** the PostgreSQL container (`foxnox-postgres-local`)
3. **Stops and removes** the Liquibase migration container (`foxnox-foxnox-migration-local`)
4. **Removes** the PostgreSQL data volume (`foxnox_postgres_data`)
5. **Restarts all services** by calling `./scripts/start-dev.sh` to rebuild and start fresh
6. **Restarts the Foxnox container** specifically to ensure clean reconnection to the database
...
The Foxnox project uses Jest as the test runner with Babel for ES module support. Tests are located in the [tests/](tests/) directory.
...
The `build-prod.sh` script builds a production-ready Foxnox Docker image using `dockerfile.prod` and `docker/conf/.env.prod`.
...
4. **Tags** the image as `dwtechs/foxnox:<version>` and `dwtechs/foxnox:latest`
...
2. **Starts** all production services in detached mode using the pre-built `dwtechs/foxnox:<version>` image
...
docker push dwtechs/foxnox:${VERSION}

# Optional: Also tag and push as 'latest' (see note below)
docker tag dwtechs/foxnox:${VERSION} dwtechs/foxnox:latest
docker push dwtechs/foxnox:latest
```
...
```bash
# Pull latest version
docker pull dwtechs/foxnox:latest

# Pull specific version
docker pull dwtechs/foxnox:1.0.0
